...
TARGETS = {
    ...
    # luc_kodi AI Search (Gemini) — escribe 3 slots + limpia caché Window de luc_kodi
    # Settings IDs definidos en plugin.video.luc_kodi v0.0.94+ bajo categoría Accounts.
    "luc_kodi_gemini":        ("luc_gemini",   "plugin.video.luc_kodi",             {
        "gemini.api.key":  GEMINI_KEY_1,
        "gemini.api.key2": GEMINI_KEY_2,
        "gemini.api.key3": GEMINI_KEY_3,
    }),
...
MENU = [
    ...
    ("[COLOR cyan]● Gemini AI[/COLOR]", [
        ("luc_kodi (AI Search)",     "gemini",       "luc_kodi_gemini"),
...
def _save_to_luc_gemini(target_addon_id, settings_dict):
    """Gemini AI keys for luc_kodi (AI Search). Same shape as _save_to_addon_multi
    but additionally invalidates luc_kodi's Window(10000)['luc_kodi_settings']
    cache, since luc_kodi's onSettingsChanged() does NOT fire on external setSetting().
    Without this clearProperty(), gemini_api_keys() would keep returning the stale
    (empty) value from cache until the user manually opens/closes settings."""
    ok, errors = _save_to_addon_multi(target_addon_id, settings_dict)
    if ok:
        _clear_luc_cache()
    return ok, errors
...
def save(provider, target_key):
    ...
    elif kind == "luc_gemini":
        ok, errors = _save_to_luc_gemini(addon_id, payload)
